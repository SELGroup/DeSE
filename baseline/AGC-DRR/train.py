...
def Train_gae(model,view_learner, data, adj, label,edge_index):
    ...
    for epoch in range(args.epoch):
        ...
        aug_adj= new_graph(torch.tensor(edge_index).to('cpu'),batch_aug_edge_weight,n,'cpu')
        aug_adj = aug_adj.to_dense()
        aug_adj = aug_adj * adj
        aug_adj = aug_adj.cpu().detach().numpy()+np.eye(n)
        aug_adj = torch.from_numpy(normalize(aug_adj)).to(torch.float32).to('cpu')
        ...
        model_loss = model.calc_loss(c.T, aug_c.T) + F.mse_loss(z_mat, torch.eye(n).to('cpu'))+ model.calc_loss(c, aug_c)
        model_loss.backward()
        optimizer.step()
        model.eval()
