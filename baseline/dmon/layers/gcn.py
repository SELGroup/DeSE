...
class GCN(Layer):  # TODO(tsitsulin): add docstring pylint: disable=missing-class-docstring
  ...
  def build(self, input_shape):
    ...
    self.kernel = self.add_weight(name='kernel', shape=(self.n_features, self.n_channels))
    self.bias = self.add_weight(name='bias', shape=(self.n_channels,))
    if self.skip_connection:
      self.skip_weight = self.add_weight(name='skip_weight', shape=(self.n_channels,))
    else:
      self.skip_weight = 0
