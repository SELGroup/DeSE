...
def deep_graph_infomax(inputs,  # pylint: disable=missing-function-docstring
    ...
    representation_summary = ReduceMeanLayer()(representations_clean)
    #representation_summary = tf.nn.sigmoid(representation_summary)
    sigmoid_layer = SigmoidLayer()
    representation_summary = sigmoid_layer(representation_summary)

    #representation_summary = tf.reshape(representation_summary, [-1, 1])
    reshape_layer = ReshapeLayer([-1, 1])
    representation_summary = reshape_layer(representation_summary)
    ...
    features_output = concat_layer([discriminator_clean, discriminator_corrupted])
